...
NAFLD<-fread('XXXXXXX/NAFLD.txt')
NAFLD<-data.frame(NAFLD)
NAFLD$id.outcome<-"NAFLD"
NAFLD$outcome<-"NAFLD"
NAFLD<-NAFLD[,c(7,5,4,11,3,14,15,13,10,9)]
colnames(NAFLD)<-c("P","SNP","effect_allele.outcome","eaf.outcome","other_allele.outcome","id.outcome","outcome","OR","se.outcome","beta.outcome")
head(NAFLD)


#修改1
for (id in available_outcomes[39550:50000,]$id){ # which(available_outcomes$id %in% c("ieu-b-40", "ieu-b-43"))  # eqtl-a-ENSG00000255150  
